...
Three themes — `light`, `dim`, `dark` — selected by a `data-theme` attribute on `<html>`, **not**
Tailwind's `dark:` variant, which is unused here. `stores/theme.ts` (TanStack Store) owns the
state, persists to `localStorage`, and is imported for side effects in `main.tsx` so the theme
applies before first paint. Every colour is a CSS variable in `index.css` mapped to Tailwind
tokens through `@theme inline`. Style with semantic classes (`bg-background`, `text-foreground`,
`text-muted-foreground`, `border-border`) — never raw hex values and never `dark:` prefixes.
